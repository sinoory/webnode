...
*
*Description:  libcurl封装上传下载模块,支持HTTP(S)、FTP
* 
*History: 
...
typedef struct _handle_s_
{
	...
	u64	size;
	u32 ticks;
	u16	speed_limit; /** 上传、下载最大速率 */
	u16	conn_limit; /** 最大线程个数 */
	u16	active_conns; /** 活动的线程个数 */
	u16	conns; /**  线程个数 */
	u8 	proto; /** 下载协议: HTTP(S) FTP */
	u8	range:1, /** 服务端是否支持断点续传 */
...
{	
	u32 u32Cnt; /** 任务个数 */
	u32 u32Limits; /** 任务最大个数 */
	handle_st **pHandle; /** 下载任务数组指针 */
	CURLSH *pDnsShare; /** dns共享数据结构 */
...
static s32 __request_fileinfo( handle_st *pstHandle )
{
	...
	fileinfo.preload = 0;
	fileinfo.pblk = apx_file_block_point( pstHandle->pFile );
...
int apx_trans_start_cl(u32 nu, struct apx_trans_glboptions* glb_opt, struct apx_trans_opt* task_opt)
{
		...
		&& ( !pstHandle->size || !pstHandle->range ))
	{
		...
		if( !s32Err )
		{
			...
			{
				task_opt->bp_continue = pstHandle->range = 0;
			}
			else
			{
...
int __download_precreate_cl( struct apx_trans_opt* task_opt )
{
	...
	if( APX_TASK_TYPE_SERVER_DOWN == task_opt->type )
	{
		task_opt->bp_continue = 0;
	}
	else
	{
