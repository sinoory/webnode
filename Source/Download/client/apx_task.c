...
#define TASK_INFO	"~/.config/cdosbrowser/appex_config/taskinfo"
...
int apx_task_uri_check(struct apx_trans_opt *trans_opt)
{
	...
	for (mode = APX_TASK_STATE_START; mode < APX_TASK_STATE_END; mode++)
	{
		list_for_each_entry(p, &userinfo->user_task.task_list[mode], list)
			...
			{
				if (trans_opt->type == APX_TASK_TYPE_SERVER_UP)
				{
					if ( (strncmp(trans_opt->fpath, p->trans_opt.fpath, sizeof(trans_opt->fpath)) == 0) && \
							 (strncmp(trans_opt->fname, p->trans_opt.fname, sizeof(trans_opt->fname)) == 0) )
					{
						ret = p->taskid;
