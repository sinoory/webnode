...
typedef	struct _apx_fileInfo_s_
{
	...
	u32	u32FNameMagic;
	u32	u32UrlMagic;
	u32	u32Upload;	/** 1: upload 2: cloud download*/
	s32	s32Pause;
	u64	u64Size;	/** 文件大小 */
	u64	u64CurSize;	/** 已下载文件大小 */
	u64	u64UpSize;	/** 下次更新blk信息时的文件大小 */
	u64	u64ProxySize; /** 云端代理下载大小 */
	s32	s32BlkCnt;	/** 数据块个数 */
...
ssize_t apx_file_read( void *pBuf, size_t szSize, s32 s32Idx, FILE_T *pFileInfo )
{
	...
	{
		flog( "parameters error( idx: %d ).", s32Idx );
		return -1;
	}
	if( pstFileInfo->s32Pause )
	{
...
*	s32Idx[IN]	块索引
*Return:
*       return 0 if success, return -4 if chekcsum failed, other return negative.
*History:
*      xyfeng     	  2015-7-13        Initial Draft 
*---------------------------------------------------*/
s32 apx_block_checksum( FILE_T *pFileInfo, s32 s32Idx  )
{
	s32 s32Ret = 0;
	size_t sLen = 0;
	s8 s8Name[FILE_LEN_MAX];
	u8 u8Buf[41] = { 0 };
	size_t sha1Len = sizeof( u8Buf );
	FILE *pFile = NULL;
	fblk_st *pstBlk = NULL;
	finfo_st *pstFileInfo = ( finfo_st* )pFileInfo;
	...
	pstBlk = &( pstFileInfo->pstBlkInfo[s32Idx] );
	sLen = strlen( pstBlk->sign );
	if( pstBlk->eState != APX_BLK_FULL
		|| 0 == sLen )
	{
		flog( "block not downloaded( idx: %d, blkCnt: %d state: %d, sign: [%s]).",
			s32Idx,  pstFileInfo->s32BlkCnt, pstBlk->eState, pstBlk->sign );
		s32Ret = -2;
		goto out;
	}
	...
	if( NULL == pFile )
	{
		flog( "failed to open `%s': %s\n", pstFileInfo->ps8FName, strerror( errno ));
		s32Ret = -3;
		goto out;
	}
	...
	SHA1_block( pFile, pstBlk->u64End -pstBlk->u64Start + 1 , u8Buf, &sha1Len );
	fclose( pFile );

	//flog( "Block[%d]: ori: %s, down: %s", s32Idx, pstBlk->sign, u8Buf );
	if( 0 != strcmp( pstBlk->sign, u8Buf ) )
	{
		flog( "block[%d] checksum faild(  ori: %s, down: %s ).", s32Idx,  pstBlk->sign, u8Buf );
		return -4;
	}
