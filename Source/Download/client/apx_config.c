...
static int apx_useragent_init(char *filename, const unsigned int filename_len, char str[], const unsigned int str_len) 
{
    FILE *fp = NULL;
    char name[64] = {0};
    char path[64] = {0};
    ...
    if (filename == NULL || str == NULL || str_len == 0)
        return -1;

    res = parse_conf_file(filename, filename_len,  name, sizeof(name), path, sizeof(path));
    if (res < 0)
        return -2;
    ...
    if (strncasecmp(str, "UserAgent:", strlen("UserAgent:")) == 0)
        memmove(str, &str[strlen("UserAgent:")], strlen(str) - strlen("UserAgent:"));
...
int apx_conf_init(char *conf_file)
{
    ...
    strncpy(useragent_file, DEFAULT_USERAGENT_FILE, sizeof(useragent_file) - 1);
    apx_useragent_init(useragent_file, sizeof(useragent_file), g_config.useragent, sizeof(g_config.useragent));
...
int apx_conf_useragent_get(char *useragent, int useragent_size)
{
    if (!g_config.conf_init_flag)
        return -1;

    if (useragent == NULL || useragent_size <= 0)
        return -2;

    if (g_config.useragent[0] != '\0')
        strncpy(useragent, g_config.useragent, useragent_size - 1);
    else 
        return -3;
