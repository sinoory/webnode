...
void
buffer_putc (struct buffer *b, unsigned char c)
{
  buffer_put(b, &c, 1);
}
...
buffer_printf(struct buffer *b, const char * fmt, ...)
{
    int len ;
    va_list arg ;
    char buff[BUFFER_SIZE_DEFAULT] ;

    va_start(arg, fmt) ;
    len = vsnprintf(buff,sizeof(buff)-1, fmt, arg) ;
    va_end(arg) ;

    if( (size_t )len >= sizeof(buff) || len < 0 )
        return -1 ;

    buffer_put(b, buff, len) ;

    return len ;
}

void 
buffer_join( struct buffer *b, struct buffer *p)
{
    struct buffer_data * d ;

    for(d = p->head; d; d=d->next )
    {
        if( d->sp >= d->cp )
            continue ;
        buffer_put(b, d->data+d->sp, d->cp - d->sp) ;
    }
    buffer_free(p);
}
  ...
  while (written > 0)
    {
      ...
      if (!(d = b->head))
        {
          //zlog_err("%s: corruption detected: buffer queue empty, "  "but written is %lu", __func__, (u_long)written);
	  break;
        }
