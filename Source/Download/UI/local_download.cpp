...
void local_download::on_pushButton_ok_clicked()
{
    ...
    struct apx_trans_opt local_opt;
    memset(&local_opt,0,sizeof(struct apx_trans_opt));
    u32 ip;
    u16 port;
    ...
    QByteArray fileId_array = fileId.toLocal8Bit();
    char *fileId_c = fileId_array.data();
    strncpy(local_opt.uri,url_c,sizeof(local_opt.uri)-1);
    strncpy(local_opt.fpath,file_path_c,sizeof(local_opt.fpath)-1);
    strncpy(local_opt.fileId,fileId_c,sizeof(local_opt.fileId)-1);
