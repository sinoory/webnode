    ...
    ui(new Ui::settings)		//初始化主界面，从settings.ini中读取相应的配置，在设置界面中显示出来
{
    ...
    QString upload = settings.value("settings/upload","0").toString();
    QString download = settings.value("settings/download","0").toString();
...
void settings::on_pushButton_save_clicked()		//点击“保存”按钮，将配置保存到settings.ini文件中
{
    ...
    int limit_result = apx_user_limit_set(uid,0,0,max_number);
    if(limit_result<0){
        qDebug()<<"uid is:"<<uid<<",limit result is:"<<limit_result;
