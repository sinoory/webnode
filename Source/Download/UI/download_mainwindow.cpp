...
void download_MainWindow::recover_task(){		//垃圾箱界面恢复任务操作
    QString name = ui->tbw_dustbin->item(row_dustbin,0)->text();
    QString size = ui->tbw_dustbin->item(row_dustbin,1)->text();
    QString path = ui->tbw_dustbin->item(row_dustbin,2)->text();
    QString file = path + "/" + name;
    int task_id = ui->tbw_dustbin->item(row_dustbin,4)->text().toInt();
    QString state = ui->tbw_dustbin->item(row_dustbin,5)->text();
    ...
    if (state == "未完成"){
        ...
        ui->tbw_dustbin->removeRow(row_dustbin);
        QString rate = "0.0B/S";
        ...
        int id = ui->tbw_dustbin->item(row_dustbin,4)->text().toInt();
        ...
        if(QFile::exists(file)){
            ...
            unsigned int download = settings1.value("settings/download",1024).toInt();
            unsigned int upload = settings1.value("settings/upload",1024).toInt();
...
void download_MainWindow::delete_filelist(){
    ...
    for(int i = 0;i<row_count;i++){
        if(ui->tbw_cloud_filelist->item(i,0)->isSelected()){
            int taskid = ui->tbw_cloud_filelist->item(i,4)->text().toInt();
...
void download_MainWindow::stop_cloud_download_task(){
    ...
    for(int i=0;i<row_count;i++){
        if(ui->tbw_cloud_download->item(i,0)->isSelected()){
            ...
            if(apx_task_stop(taskid)<0)
                log.uilog_write("stop task failed,task id is:" + QString::number(taskid));
        }
    }
}
...
void download_MainWindow::hand_Message(){	//接收线程发出的信号，更新主界面上的信息
    ...
    for(int k=0;k<=row1;k++){
        ...
        if(finish_size==100 ||file_state==APX_TASK_STATE_FINISHED ){
             ...
             else if(total_size<1024 &&total_size>=0){
...
void download_MainWindow::update_cloud_download(){
    ...
    for(int i = 0;i<row;i++){
        ...
        apx_cloud_task_status(taskid,&httpCode,&taskinfo);
...
void download_MainWindow::update_cloud_upload(){
    ...
    for(int i = 0;i<row;i++){
        ...
        if(finish_size==100 || task_state==APX_TASK_STATE_FINISHED){
            ...
            QString size = ui->tbw_cloud_upload->item(i,1)->text();
            QDateTime d_time = QDateTime::currentDateTime();
            ...
            ui->tbw_cloud_filelist->setItem(0,1,new QTableWidgetItem(QString(size)));
            ui->tbw_cloud_filelist->setItem(0,2,new QTableWidgetItem(QString(date_time)));
            ui->tbw_cloud_filelist->setItem(0,3,new QTableWidgetItem(QString(file_id)));
            ui->tbw_cloud_filelist->setItem(0,4,new QTableWidgetItem(QString::number(Id)));
            ui->tbw_cloud_upload->removeRow(i);
            write_all_config();
            return;
        }
...
void download_MainWindow::get_cloud_list(){
    ...
    apx_cloud_task_list(&http_code,0,0,&tasklist);
    ...
    for(int i=0;i<task_number;i++){
        ...
        ui->tbw_cloud_filelist->setItem(i,1,new QTableWidgetItem(QString::number(size)));
