...
void addnew::on_pushButton_ok_clicked()		//当填写完下载信息之后点击确定，执行一下操作
{
    ...
    if(ui->radioButton_http->isChecked()){		//当下载任务是http下载时，执行的操作
            ...
            unsigned int upload = settings.value("settings/upload",0).toInt();
            unsigned int download = settings.value("settings/download",0).toInt();
            struct apx_trans_opt addnew_opt;
            if(ui->chk_cloud_accelerate->isChecked())
                addnew_opt.cloud_proxy = 1;
            ...
            if(download_referer!=""){
                if(download_referer.left(8)!="Referer:")
                    download_referer = "Referer: " + download_referer;
                QByteArray referer_array = download_referer.toLocal8Bit();
                char *referer_char = referer_array.data();
                addnew_opt.referer = new char[5000];
                strncpy(addnew_opt.referer,referer_char,5000);
                qDebug()<<"referer is:"<<addnew_opt.referer;
                download_referer="";
            }
            ...
            if(ui->chk_offline->isChecked()){
                ...
                memset(&taskinfo,0,sizeof(cld_taskinfo_st));
                addnew_opt.proto = APX_TASK_PROTO_HTTP;
                task_id = apx_cloud_task_create(&addnew_opt,&taskinfo);
                ...
                if(task_id<0){
                    ...
                    fileId = QString((char *)taskinfo.fileId);
                    done(3);
                    return;
                }
    ...
    if(ui->radioButton_bt->isChecked()){			//当下载任务是bt下载时执行的操作
        ...
        unsigned int upload = settings.value("settings/upload",0).toInt();
        unsigned int download = settings.value("settings/download",0).toInt();
        qDebug()<<"download speed is :" <<download;
        struct apx_trans_opt addnew_opt;
        memset(&addnew_opt,0,sizeof(struct apx_trans_opt));
        addnew_opt.concurr = one_max_thread;
        addnew_opt.type = task_type;
        ...
        task_id  = apx_task_create(&addnew_opt);
        qDebug()<<"path:"<<addnew_opt.fpath<<"conn:"<<addnew_opt.concurr<<"download_limit:"<<addnew_opt.down_splimit<<"fname:"<<addnew_opt.fname<<"proto:"<<addnew_opt.proto;
        if(task_id==-1){
            QMessageBox msgbox(QMessageBox::NoIcon,tr("错误"),tr("uid不存在"));
            msgbox.setStandardButtons(QMessageBox::Ok);
            msgbox.setButtonText(QMessageBox::Ok,QString("确定"));
    ...
    if(ui->radioButton_ftp->isChecked()){			//当下载任务是ftp下载时，执行的操作
            ...
            if(ui->chk_offline->isChecked()){
                cld_taskinfo_st taskinfo;
                memset(&taskinfo,0,sizeof(cld_taskinfo_st));
                addnew_opt.proto = APX_TASK_PROTO_FTP;
                task_id = apx_cloud_task_create(&addnew_opt,&taskinfo);
...
void addnew::on_chk_offline_clicked()
{
    if(ui->chk_offline->isChecked())
        ui->chk_cloud_accelerate->setChecked(false);
}

void addnew::on_chk_cloud_accelerate_clicked()
{
    if(ui->chk_cloud_accelerate->isChecked())
        ui->chk_offline->setChecked(false);
}
